...
export class RestoreProductUseCase extends AbstractUseCase<
  OneProductDto.Input,
  void
> {
  public constructor(
  ...
  public async execute(input: OneProductDto.Input): Promise<void> {
    ...
    if (product.deletedAt === null)
      throw new TRPCError({
        code: 'BAD_REQUEST',
        message: `Product with ID ${input.id} is not deleted`,
      })

    const restoredProduct = product.clone({ deletedAt: null })
    await this._productRepo.save(restoredProduct)
  }
}
